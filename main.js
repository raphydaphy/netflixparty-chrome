function netflixParty() {
  if (window.netflixPartyLoaded) return;
  window.netflixPartyLoaded = true;

  console.log("Loading Netflix Party+");
  ...
  // Socket.IO v2.3.0 | (c) 2014-2019 Guillermo Rauch | Released under the MIT License.
  ...
  // An array of the default icon names
  ...
  function createIconButton(i) {
    var iconButton = jQuery("#icon-holder").append(`
      <a class="image-button">
        <img class="unselectable img-class" src="${getIconURL(icons[i])}" id="user-icon-${icons[i]}">
      </a>
    `);
